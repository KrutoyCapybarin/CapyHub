--[[
================================================================
  UI LIBRARY  —  globals added after loading: UI, Flags
================================================================

СТРУКТУРА: Window → Tab → Section → Widget
...
local win = UI:Window({
    Title  = "My Window",          -- заголовок
    Size   = Vector2.new(700,500), -- размер
    Open   = true,                 -- показать сразу (default: true)
    Flags  = {},                   -- { Flags.Window.NoMove, Flags.Window.NoCollapse }
})

win:Toggle()          -- спрятать/показать окно
win:Toggle(true/false)-- явно задать видимость
win:Unload()          -- полностью уничтожить (X в тайтлбаре делает то же самое)

win.Internal.Running  -- false после Unload(), используй в while-петле
...
local tab = win:Tab({
    Title = "Tab Name",
    Flags = {},        -- { Flags.Tab.Inactive, Flags.Tab.NoScroll }
})

local sec = tab:Section({
    Title     = "Section Name",
    Collapsed = false, -- свёрнута по умолчанию? (default: false)
    Flags     = {},    -- { Flags.Section.Inactive, Flags.Section.NoCollapse }
})

----------------------------------------------------------------
  ВИДЖЕТЫ (добавляются в секцию)
----------------------------------------------------------------

-- Текст (просто лейбл)
local lbl = sec:Label({ Title = "Hello" })
lbl:Get()             -- -> string
lbl:Set("New text")

-- Кнопка
sec:Button({ Title = "Click me" }, function()
    -- вызывается при клике
end)

-- Чекбокс
local cb = sec:Checkbox({ Title = "Enable", Default = false }, function(bool)
...
local sl = sec:Slider({
    ...
    Step    = 1,
    Default = 50,
    Suffix  = "px",   -- текст после значения, например "px", "°" (можно "")
}, function(number)
    -- вызывается при изменении
end)
sl:Get()              -- -> number
...
-- Мультидропдаун (несколько вариантов)
local mdd = sec:MultiDropdown({
    Title   = "Items",
    Options = { "X", "Y", "Z" },
    Default = { "X" },
}, function(selectedTable)
    -- вызывается при изменении, selectedTable — { string }
end)
mdd:Get()             -- -> { string }
mdd:Set({ "X", "Z" })
mdd:Add("Y")
mdd:Remove("Y")
mdd:Toggle("Y")       -- добавит если нет, удалит если есть

-- Кейбинд
local kb = sec:Keybind({
    Title = "Fly",
    Key   = Enum.KeyCode.F,
    Flags = {},        -- { Flags.Widgets.Keybind.NoChange } — запрет смены клавиши
},
function()             -- вызывается при нажатии клавиши
    -- ...
end,
function(newKey)       -- вызывается когда пользователь сменил клавишу
    -- newKey = Enum.KeyCode или nil (Escape = сброс)
end)
...
Flags.Window.NoMove          -- нельзя двигать окно
Flags.Window.NoCollapse      -- нельзя свернуть окно

Flags.Tab.Inactive           -- вкладка недоступна (серая)
Flags.Tab.NoScroll           -- отключить скролл в вкладке

Flags.Section.Inactive       -- секция недоступна
Flags.Section.NoCollapse     -- нельзя свернуть секцию

Flags.Widgets.Keybind.NoChange  -- нельзя сменить клавишу кейбинда

-- Установить флаг:
win.Flags.Set(Flags.Window.NoMove, true)
tab.Flags.Set(Flags.Tab.Inactive, false)
sec.Flags.Set(Flags.Section.NoCollapse, true)
kb.Flags.Set(Flags.Widgets.Keybind.NoChange, true)
...
function UI:Window(Options)
	...
	Window.Children.Drawings.CloseButton = {
		Enabled = true,
		Drawing = CloseButton,
	}
	...
	CloseText.ZIndex = -16
	CloseText.Text = "x"
	Window.Children.Drawings.CloseText = {
		Enabled = true,
		Drawing = CloseText,
	}
	...
	local OnPress = Helpers.OnPress(function()
		...
		if Helpers.IsMouseInRect(CloseButton.Position, CloseButton.Size) then
			Window:Unload()
		elseif not Window.Flags.Has(Flags.Window.NoCollapse) and (Helpers.IsMouseInRect(Helpers.GetDrawingBounds(Collapse).Position, Helpers.GetDrawingBounds(Collapse).Size)) then
	...
	Window.Update = function()
		...
		-- Close button: square in top-right of title bar
		local CloseSize = Top.Size.Y - 6
		CloseButton.Size = Vector2.new(CloseSize, CloseSize)
		CloseButton.Position = Top.Position + Vector2.new(Top.Size.X - CloseSize - 3, 3)
		CloseButton.Color = Color3.fromRGB(140, 40, 40)
		CloseButton.Corner = Window.Theme.Global.Corner

		local CloseBounds = Helpers.GetTextBounds(CloseText)
		CloseText.Position = CloseButton.Position + Vector2.new(
			CloseButton.Size.X / 2 - CloseBounds.X / 2,
			CloseButton.Size.Y / 2 - CloseBounds.Y / 2
		)
		CloseText.Color = Window.Theme.Global.Text

		-- Collapse arrow shifted left to avoid overlapping close button
		...
		if Window.Internal.Collapsed then
			Collapse.PointA = Base.Position + Vector2.new(Base.Size.X - 38, 20)
			Collapse.PointB = Collapse.PointA + Vector2.new(-8, -15)
			Collapse.PointC = Collapse.PointA + Vector2.new(8, -15)
		else
			Collapse.PointA = Base.Position + Vector2.new(Base.Size.X - 45, 13)
		...
		-- Always-visible title bar elements (visible even when collapsed)
		local AlwaysVisible = {
			Window.Children.Drawings.Top,
			Window.Children.Drawings.Collapse,
			Window.Children.Drawings.Title,
		...
		for Name, Drawing in Window.Children.Drawings do
			if table.find(AlwaysVisible, Drawing) then
...
function Section:Checkbox(Options, Callback)
	...
	Checkbox.Update = function(index)
		...
		CheckTop.Color = Color3.fromRGB(255, 255, 255)
